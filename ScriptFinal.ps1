...
$Users = Import-Csv -Path 'C:\Users\Administrator\Desktop\s09_SocieteBillU.xlsx_-_Sheet1.csv'
...
$csv = Import-Csv -Path "C:\Users\Administrator\Desktop\BillU.csv" -Delimiter "," -header "Prenom" , "Nom","Société","Site",`
                "Département","Service","fonction","Manager - prénom","Manager - nom","Date de naissance","email","Téléphone fixe","Téléphone portable" `
                ,"Nomadisme - Télétravail" -Encoding UTF8 | Select-Object -Skip 1
$csv = $csv  |Select-Object -Property Service, Département | Export-Csv -Path 'C:\Users\Administrator\Desktop\BillU.csv' -NoTypeInformation
...
Foreach ($OU in $OUs)
{
    CreateSousOUFunction -OU $OU.Service -SearchBase $OU.Département
}
}
...
Function Groupe
{
Creategrp1 ; FormatCsv ; DptSrv ; Creategrp2
}
...
{
# Importer le fichier CSV initial
$Users = Import-Csv -Path 'C:\Users\Administrator\Desktop\s10_SocieteBillU.xlsx_-_Sheet1_1.csv'
...
Foreach ($User in $Users)
{
     ...
     If (($ADUsers | Where {$_.SamAccountName -eq $SamAccountName}))
    {
        ...
        Write-Host "Ajout d'informations à l'Utilisateur $SamAccountName" -ForegroundColor Green
    }
    Else
    {
