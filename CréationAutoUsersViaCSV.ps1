...
clear
$Users = Import-Csv -Path 'C:\Users\Administrator\Desktop\s09_SocieteBillU.xlsx - Sheet1 (1).csv'
...
Foreach ($User in $Users)
{
    ...
    if ($($user.Service) -eq "")
    {
        $path = "ou=$($user.'D?partement'),dc=lab,dc=lan"
    }
    else
    {
        $path = "ou=$($user.service),ou=$($user.'D?partement'),dc=lab,dc=lan"
    }
    $Company           = "BillU"
